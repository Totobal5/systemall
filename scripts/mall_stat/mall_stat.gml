...
function MallStat(_statKey) : MallState(_statKey) constructor 
{
	...
	setFunEquip   ("fStatEquip00"); // Default
	...
	/// @desc Guardar valor en PartyStat
	static toggleSValue = function() 
	{
		saveValue = true;
		return self;
	}
	...
	static inherit = function(_KEY, _LIMIT=true, _LVL=true)
	{
		...
		// Hereda nivel
		if (_LVL) {setLevel(_stat.levelLimitMin, _stat.levelLimitMax, _stat.funLevel, _stat.levelSingle, _stat.checkLevel); }
...
function mall_add_stat() 
{
	...
	static DebugMessage = MallDatabase.statsDebugMessage;
    ...
    var i=0; repeat(argument_count) {
		...
		if (!variable_struct_exists(stats, _key) ) {
			...
			if (MALL_TRACE) DebugMessage("(AddStat): " + _key + " added");
		}
...
/// @param  {Bool}          [inheritLimits]    Heredar limites de valores
/// @param  {Bool}          [inheritLevel]     Heredar configuracion de nivel
/// @param	{String}        [displayKey]       Llave para traducciones en lexicon
/// @returns {Struct.MallStat}
