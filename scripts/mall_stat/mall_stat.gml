...
function MallStat(_key) : MallEvents(_key) constructor 
{
    ...
    // Valor base con el que empieza la estadística.
    base_value = 0;
    ...
    {
        // El operador '??' usa el valor de la izquierda si existe, si no, usa el de la derecha (valor por defecto).
...
function mall_get_stat(_statKey) { return (Systemall.__stats[$ _statKey] ); }

/// @param {String} key
function mall_exists_stat(_statKey) { return (struct_exists(Systemall.__stats, _statKey) ); }

/// @desc Devuelve un array con las llaves de todos las estadisticas creadas
/// @return {Array<String>}
function mall_get_stat_keys() { return (Systemall.__stats_keys); }