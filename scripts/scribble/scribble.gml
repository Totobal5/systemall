...
/// 
/// @param _string       The string to parse and, eventually, draw
/// @param [_unique_id]   A unique identifier that can be used to distinguish this occurrence of the input string from other occurrences. Only necessary when you might be drawing the same string at the same time with different animation states
/// @return {Struct.__scribble_class_element}
function scribble(_string, _unique_id)
{
	_unique_id ??= SCRIBBLE_DEFAULT_UNIQUE_ID;
    ...
    var _weak = global.__scribble_ecache_dict[$ string(_string) + ":" + string(_unique_id)];
    if ((_weak == undefined) || !weak_ref_alive(_weak) || _weak.ref.__flushed)
    {
        return (new __scribble_class_element(string(_string), string(_unique_id) ) );
    }
    else
    {
        return _weak.ref;
